...
/*   Updated: 2024/11/26 10:24:09 by hait-sal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void insertPend(vvi& mainChain, vvi& pend) {
    vvi::iterator it_pend = pend.begin();
    for (int i = 0; it_pend != pend.end(); ++it_pend) {
        vvi::iterator it = std::lower_bound(mainChain.begin(), mainChain.end(), *it_pend, compare);
        int pos = it - mainChain.begin();
        mainChain.insert(mainChain.begin() + pos, *it_pend);
        ++i;
    }
}
...
void elInsert(vi& toSort, int elSize) {
    ...
    if (chain.size() > 3) { //insertion for a chain which is more than 3
        ...
        // std::cout << pend[0].first.back() << std::endl;
        // std::cout << pend[1].first.back() << std::endl;
        // std::cout << pend[0].first << std::endl;
        // std::cout << pend[0].first << std::endl;
        // std::cout << pend[0].first << std::endl;
        // print_vp(pend);
